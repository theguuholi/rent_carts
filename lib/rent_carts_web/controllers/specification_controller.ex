defmodule RentCartsWeb.SpecificationController do
  use RentCartsWeb, :controller

  alias RentCarts.Specifications
  ...
  def create(conn, %{"specification" => specification_params}) do
    with {:ok, %Specification{} = specification} <-
      ...
      conn
      |> put_status(:created)
      |> put_resp_header("location", Routes.specification_path(conn, :show, specification))
